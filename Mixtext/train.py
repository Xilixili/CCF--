...
parser.add_argument('--mix-layers-set', nargs='+',
                    default=[7,9,12], type=int, help='mix的层的集合，指定那些层做mix')
...
def main():
    global best_acc
    # 读取 dataset 和构建 dataloaders，调用的readdata文件里面的getdata函数
    ...
    val_loader = Data.DataLoader(dataset=val_set, batch_size=512, shuffle=False)
    test_loader = Data.DataLoader(dataset=test_set, batch_size=512, shuffle=False)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, scheduler, criterion, epoch, n_labels, train_aug=False):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        # 第一步
        # 这里返回参数个数不同是因为在getitem函数中针对增强和非增强返回的参数是不同的
        #如果训练集不做数据增强
        if not train_aug:
            print('不做增强')
        ...
        #如果使用数据增强，拼接所有输入，长度和标签（无标签数据预测出来的标签)
            ...
            all_inputs = torch.cat([inputs_x, inputs_x_aug, inputs_u, inputs_ori], dim=0)
            all_lengths = torch.cat([inputs_x_length, inputs_x_length, length_u, length_ori], dim=0)
        ...
        if args.mix_method == 0:
            ...
            if lbeta == 1:
                ...
                mixed = 1
        #使用SemiLoss计算损失，Lx表示L(Tmix)，Lu表示L(margin)
...
mixed_target[batch_size:-batch_size_2], logits[-batch_size_2:], epoch+batch_idx/args.val_iteration, mixed)
        ...
        if batch_idx % 1000 == 0:
            print("epoch {}, step {}, loss {}, Lx {}, Lu {}, Lu2 {}".format(epoch, batch_idx, loss.item(), Lx.item(), Lu.item(), Lu2.item()))
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, outputs_u_2, epoch, mixed=1):
        ...
        if args.mix_method == 0 or args.mix_method == 1:
            #有监督的x的损失
            Lx = - \
                torch.mean(torch.sum(F.log_softmax(outputs_x, dim=1) * targets_x, dim=1))
            #无监督的x输出的概率值
            probs_u = torch.softmax(outputs_u, dim=1)
            #论文中公式显示的kl散度, batch mean 批次均值作为统计计算KL散度
            Lu = F.kl_div(probs_u.log(), targets_u, None, None, 'batchmean')
            #计算hinge Loss 折页损失 max(0,1-(wTxi +b)yi)
            Lu2 = torch.mean(torch.clamp(torch.sum(-F.softmax(outputs_u, dim=1) * F.log_softmax(outputs_u, dim=1), dim=1) - args.margin, min=0))

        elif args.mix_method == 2:
            if mixed == 0:
                Lx = - \
                    torch.mean(torch.sum(F.logsigmoid(outputs_x) * targets_x, dim=1))
                ...
                Lu2 = torch.mean(torch.clamp(args.margin - torch.sum(F.softmax(outputs_u, dim=1) * F.softmax(outputs_u, dim=1), dim=1), min=0))
